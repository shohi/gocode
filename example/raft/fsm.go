...
func (s *Store) Open(enableSingle bool, localID string) error {
	...
	s.obCh = make(chan raft.Observation, 1024)
	ra.RegisterObserver(raft.NewObserver(s.obCh, true, nil))
	s.raft = ra
...
func (s *Store) Apply(data []byte, timeout time.Duration) error {
	f := s.raft.Apply(data, timeout)
	return f.Error()
}

func (s *Store) IsLeader() int {
...
func (s *Store) Leave() error {
	future := s.raft.RemoveServer(raft.ServerID(s.serverID), 0, 0)
...
func (s *Store) start() {
	...
	go func() {
		for range c {
			if count%50 == 0 {
				// stats := s.raft.Stats()
				ff := s.raft.GetConfiguration()
				if ff.Error() == nil {
					peers := ff.Configuration().Servers
					log.Printf("ticker ===> ID: %d/%d, count: %d, servers: [%v]",
						s.numericalID, len(peers), count, peers)
				}
			}
			count++
		}
	...
	go s.handleRaftEvents()
}

func (s *Store) handleRaftEvents() {
	for ob := range s.obCh {
		event := ob.Data
		switch event.(type) {
		case raft.PeerObservation:
			log.Printf("raft ====> PeerObservation: [%v]\n", event)

		case raft.LeaderObservation:
			log.Printf("raft ====> LeaderObservation: [%v]\n", event)

		case raft.RaftState:
			log.Printf("raft ====> RaftState: [%v]\n", event)

		default:
			log.Printf("raft ====> OtherObservation: [%v]\n", event)
		}
	}
}
...
func (f *fsm) Apply(l *raft.Log) interface{} {
	...
	log.Printf("apply===> serverID: [%v], peerID: [%v], data: [%v]\n",
		ID, f.numericalID, string(l.Data))
